...
import { useRequest, type PageEvent } from "solid-start/server";
import type {
  AuthClientConfig,
  LiteralUnion,
...
import { conditionalEnv, getEnv, now, parseUrl } from "./utils";
...
export const SessionContext = createContext<Resource<Session | null> | undefined>(undefined);
...
export function createSession(): Resource<Session | null> {
  ...
  if (!value && (import.meta as any).env.DEV) {
    throw new Error("[~/auth]: `createSession` must be wrapped in a <SessionProvider />");
  }
